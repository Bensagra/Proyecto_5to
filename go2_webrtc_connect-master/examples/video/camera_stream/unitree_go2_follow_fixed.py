...
# En v2.0.4 las clases no se exportan desde __init__.py, hay que importar del submódulo
...
class RescueDogApp:
    ...
    def __init__(self, cfg: Config):
        ...
        self.conn = UnitreeWebRTCConnection(method, ip=cfg.robot_ip)
    ...
    async def _recv_camera_stream(self, track):
        """
        Recibe frames del MediaStreamTrack del Go2 y los encola para el SSD.
        Patrón correcto según ejemplos oficiales de unitree_webrtc_connect v2.0.4.
        """
        ...
        while self._running:
            try:
                ...
                log.debug("Error recibiendo frame: %s", e)
    ...
    async def run(self):
        ...
        # 1) Conectar al robot PRIMERO (igual que en el ejemplo oficial)
        log.info("Conectando al Go2 (%s)...", self.cfg.robot_ip)
        await self.conn.connect()
        log.info("Conexión establecida ✓")

        # 2) Activar canal de video y registrar track callback
        #    add_track_callback recibe un MediaStreamTrack, NO un frame directo
        try:
            self.conn.video.switchVideoChannel(True)
            self.conn.video.add_track_callback(self._recv_camera_stream)
            log.info("Canal de video activado ✓")
        except AttributeError as e:
            log.warning("Video no disponible: %s", e)
        ...
        try:
            ...
            log.warning("LiDAR no disponible: %s", e)
