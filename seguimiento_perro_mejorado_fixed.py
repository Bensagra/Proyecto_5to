...
║                                                                              ║
║     USO:                                                                    ║
║     python seguimiento_perro_mejorado_fixed.py --ip 192.168.8.181          ║
║     python seguimiento_perro_mejorado_fixed.py --ip 192.168.8.181 --debug  ║
║     python seguimiento_perro_mejorado_fixed.py --ap                        ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class Config:
    ...
    max_forward_speed: float = 1.8          # m/s adelante/atrás (reducido para seguridad)
    min_forward_speed: float = 0.8          # m/s velocidad mínima
    max_turn_speed: float = 0.55            # rad/s giro máximo
    
    # ── Ganancias PD para control ────────────────────────────────────────
    kp_forward: float = 8.5                 # proporcional distancia
    kp_turn: float = 0.75                   # proporcional giro
    smoothing_alpha: float = 0.20           # suavizado exponencial (0-1)
    smooth_error_alpha: float = 0.70        # suavizado de error medido
    
    # ── LiDAR para evitar obstáculos ─────────────────────────────────────
    lidar_enabled: bool = True
    lidar_emergency_stop_m: float = 0.50    # obstáculo < 50cm → STOP total
    lidar_slow_zone_m: float = 1.00         # obstáculo < 100cm → reducir vel
    lidar_front_angle_deg: float = 65.0     # cono frontal analizado
    lidar_height_min: float = -0.10         # filtro altura mínima
    lidar_height_max: float = 1.60          # filtro altura máxima
    lidar_min_points: int = 8               # mínimo puntos para análisis válido
    ...
    connection_timeout: float = 15.0        # timeout para conexión inicial
...
async def main_async(cfg: Config, conn: Go2WebRTCConnection):
    ...
    try:
        await asyncio.wait_for(conn.connect(), timeout=cfg.connection_timeout)
        log.info("[MAIN] Conectado ✓")
    except asyncio.TimeoutError:
        log.error("[MAIN] ✗ Timeout conectando al robot")
        log.error("[MAIN] Verifica que:")
        log.error("  - El robot esté encendido")
        log.error("  - La IP sea correcta")
        log.error("  - Estés en la misma red que el robot")
        return
    except Exception as e:
        log.error(f"[MAIN] ✗ Error conectando: {e}")
        return
    
    # Activar cámara
    try:
        conn.video.switchVideoChannel(True)
        log.info("[CAMERA] Stream activado")
    except Exception as e:
        log.error(f"[CAMERA] Error activando: {e}")
    ...
    log.info("")
    log.info("╔════════════════════════════════════════════════════════════════╗")
    log.info("║  ROBOT LISTO - Presiona 'Q' para salir, 'S' para stop         ║")
    log.info("╚════════════════════════════════════════════════════════════════╝")
    log.info("")
...
1. CON IP CONOCIDA (MÁS RECOMENDADO):
   python seguimiento_perro_mejorado_fixed.py --ip 192.168.0.114
   python seguimiento_perro_mejorado_fixed.py --ip 192.168.8.181
   python seguimiento_perro_mejorado_fixed.py --ip 192.168.12.1

2. MODO AP (Si el robot tú mismo es WiFi):
   python seguimiento_perro_mejorado_fixed.py --ap

3. AUTODISCOVERY (Busca por serial):
   python seguimiento_perro_mejorado_fixed.py

OPCIONES ADICIONALES:
   --debug              Ver mensajes de debugSin LiDAR:
   --no-lidar           Desactivar LiDAR
   --no-face-save       No guardar detecciones de caras

TIPS:
   - Verifica la IP del robot en su pantalla frontal
   - Prueba conectividad: ping <IP>
   - El robot debe estar en la misma red que tu laptop
        """
    )
    parser.add_argument(
        "--ip",
        type=str,
        default=None,
        help="IP del robot (ej: 192.168.0.114, 192.168.8.181, 192.168.12.1)"
    )
    parser.add_argument(
        "--serial",
        type=str,
        default="B42D2000P7I9GF8A",
        help="Serial number del robot (alternativa a --ip)"
    )
    parser.add_argument(
        "--ap",
        action="store_true",
        help="Usar modo AP (robot como punto de acceso 192.168.12.1)"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    # Configurar método de conexión según argumentos
    if args.ap:
        cfg.connection_method = "AP"
        cfg.robot_ip = "192.168.12.1"
        log.info("[CONFIG] Modo: AP (LocalAP) - Robot como punto de acceso")
    else:
        cfg.connection_method = "STA"
        if args.ip:
            cfg.robot_ip = args.ip
            log.info(f"[CONFIG] Modo: STA (IP especificado) - {args.ip}")
        else:
            cfg.robot_ip = None  # Intentar autodiscovery con serial
            log.info("[CONFIG] Modo: STA (Autodiscovery) - Buscando robot por serial...")
    ...
    try:
        if cfg.connection_method == "AP":
            log.info("[CONEXIÓN] Intentando LocalAP (192.168.12.1)...")
            conn = Go2WebRTCConnection(WebRTCConnectionMethod.LocalAP)
        else:
            # STA mode - intentar con IP si está disponible, sino con serial
            if cfg.robot_ip:
                log.info(f"[CONEXIÓN] Intentando LocalSTA con IP {cfg.robot_ip}...")
                conn = Go2WebRTCConnection(
                    WebRTCConnectionMethod.LocalSTA,
                    ip=cfg.robot_ip
                )
            else:
                log.info(f"[CONEXIÓN] Intentando LocalSTA con serial {cfg.serial_number}...")
                conn = Go2WebRTCConnection(
                    WebRTCConnectionMethod.LocalSTA,
                    serialNumber=cfg.serial_number
                )
        
        log.info("[CONEXIÓN] Objeto de conexión creado ✓")
        ...
        log.error("")
        log.error("1. PRUEBA CON IP CONOCIDA (RECOMENDADO):")
        log.error("   python seguimiento_perro_mejorado_fixed.py --ip 192.168.0.114")
        log.error("   python seguimiento_perro_mejorado_fixed.py --ip 192.168.8.181")
        log.error("   python seguimiento_perro_mejorado_fixed.py --ip 192.168.12.1")
        log.error("")
        log.error("2. MODO AP (Si el robot es punto de acceso):")
        log.error("   python seguimiento_perro_mejorado_fixed.py --ap")
        log.error("")
        log.error("3. VERIFICA:")
        log.error("   - El robot esté encendido")
        log.error("   - Estés en la misma red WiFi")
        log.error("   - Prueba: ping <IP>")
        log.error("   - Ver IP del robot en su pantalla frontal")
        return
    
    if conn is None:
        log.error("✗ No se pudo crear conexión")
        return
    ...
    try:
        ...
        log.info("[MAIN] Interrumpido por teclado")
    except Exception as e:
        log.error(f"✗ Error en main_async: {e}", exc_info=args.debug)
    finally:
